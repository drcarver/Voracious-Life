...
namespace Voracious.Control.FileWizards;

class EpubWizard
{
    private static ILogger? logger;

    EpubWizard(IServiceProvider servicesProviders)
    {
        var lgf = servicesProviders?.GetRequiredService<ILoggerFactory>();
        logger = lgf.CreateLogger<EpubWizard>();
    }
    ...
    public static (string value, int index, string filename, string foundId) FindHtmlContainingId(EpubBookExt EpubBook, List<string> idList, int preferredHtmlIndex)
    {
        foreach (var id in idList)
        {
            if (preferredHtmlIndex >= 0)
            {
                ...
                var str = Encoding.UTF8.GetString(html.Content);
        ...
        if (idList[0] != "uiLog")
        {
            throw new Exception($"ERROR: unable to find html containing id={idList[0]} in the ebook");
        }
    ...
    public static int HtmlStringIdIndexOf(string html, string id, bool allowHref = false)
    {
        ...
        while (startidx >= 0)
        {
            ...
            if (idx >= 4)
            {
                ...
                if (html[eqIndex] != '=')
                {
                    logger?.LogError($"ERROR in HtmlStringContainsId at index {eqIndex} expected equals = ");
                }
                var tagIndex = SkipWsBack(html, eqIndex - 1);
                if (tagIndex < 0) continue;
                if (tagIndex < 2)
                {
                    logger?.LogError($"ERROR in HtmlStringContainsId at index {eqIndex} expected enough room for a tag {tagIndex} ");
                    continue;
                }
                ...
                switch (partial)
                {
                    ...
                    case "ef": // part of href=
                        ...
                        {
                            logger?.LogInformation($"NOTE: EpubWizard: Find({id}) at pos {ltIndex} nodeType={nodeType} but reject because we don't like src=");
                            if (allowHref)
                            {
                                return idx;
                            }
                    ...
                    case "by":
                        ...
                        logger?.LogInformation($"NOTE in HtmlStringContainsId at index {eqIndex} expected id or (s)rc not {partial} for nodetype={nodeType}");
                        break;
                    default:
                        logger?.LogError($"ERROR in HtmlStringContainsId at index {eqIndex} expected id or (s)rc not {partial} ");
                        break;
                }
            }
        }
    ...
    {
        int nerror = 0;
        var actual = HtmlStringIdIndexOf(html, id) >= 0;
        if (actual != expected)
        {
            nerror++;
            logger?.LogError($"ERROR: HtmlStringContainsId({html}, {id}) expected {expected} but got {actual}");
        }
        return nerror;
    }
    public static string FindHtmlByIndex(EpubBookExt epubBook, int searchIndex)
    {
        ...
        logger?.LogError($"MainEpubReader: FindHtmlByIndex: can't find html index {searchIndex}");
        if (lastcontent != null)
        {
            return Encoding.UTF8.GetString(lastcontent);
        }
        return null;
    }
    ...
    public static EpubFile GetInternalFileByName(EpubBookExt epubBook, string requestFileName)
    {
        ...
        logger?.LogError($"MainEpubReader: GetInternalFileByName({requestFileName}) can't find requested file");
        return null;
    }
    ...
    public static string GetChapterContainingId(EpubBookExt epubBook, string id, int preferredHtmlIndex)
    {
        ...
        var idList = GetIdVariants(id);
        var (foundHtml, foundIndex, foundHtmlName, foundId) = FindHtmlContainingId(epubBook, idList, preferredHtmlIndex);
        if (foundHtml == null)
        {
            if (id != "uiLog")
            {
                // uiLog isn't always findable for ... reasons
                logger?.LogError($"IMPOSSIBLE ERROR: completely unable to find id {id} ");
            }
            return null;
        }

        var pos = HtmlStringIdIndexOf(foundHtml, id);
        string closest = null;
        ...
        if (closest == null)
        {
            logger?.LogError($"ERROR: when asked for matching chapter, can't find it for {id}. Possibly the chapters have no anchors.");
        }
    ...
    public static (string value, int index, string filename) FindHtmlContainingHtmlFileName(EpubBookExt epubBook, string htmlFileName)
    {
        ...
        logger.LogError($"MainEpubReader: FindHtmlContainingHtmlFileName: can't find {htmlFileName}");
        return (null, -1, null);
    }
