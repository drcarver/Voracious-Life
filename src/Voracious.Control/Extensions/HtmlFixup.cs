...
public class HtmlFixup
{
    ...
    HtmlFixup(ILoggerFactory loggerFactory, IConfiguration JsonConfiguration)
    {
        logger = loggerFactory.CreateLogger(nameof(HtmlFixup));
...
//]]>
    ...
    private static string FixupBodyPadding(string html)
    {
        ...
        var fontFamily = configuration.GetSection("FontFamilyName");
        var fontSize = configuration.GetSection("FontSizeHtml");
        ...
        if (idx < 0)
        {
            logger.LogError($"ERROR: MainEpubReader: FixupBodyPadding: can't find the <body tag!");
            return html;
        }
    ...
    private static string FixupImgAndLink(string html)
    {
        ...
        if (nimgReplaced != nImgTags)
        {
            if (nImgTags > 0)
            {
                logger.LogError($"NOTE: replaced {nimgReplaced} but there were {nImgTags}");
            }
            ; // an error
        }
        return html;
