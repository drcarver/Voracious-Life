...
public partial class ResourceViewModel : ObservableObject, IResource, IGetSearchArea
{
    ...
    /// </remarks>
    [ObservableProperty]
    private List<IFileFormat> formats = [];
    ...
    public string? BestAuthorDefaultIsNull => Creators.Cast<Creator>().OrderBy(p => p.GetImportance()).FirstOrDefault()?.Name;
