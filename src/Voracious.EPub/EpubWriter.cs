...
using Voracious.EPub.Format.Writers;
...
public class EpubWriter
{
    ...
    public EpubWriter()
    {
        var opf = new OpfDocument
        {
            ...
            EpubVersion = EpubVersionEnum.Epub3
        };
        ...
        opf.Manifest.Items.Add(new OpfManifestItem { Id = "ncx", Href = "toc.ncx", MediaType = ContentType.ContentTypeToMimeType[EpubContentEnum.DtbookNcx] });
        opf.Spine.Toc = "ncx";
    ...
    public void AddFile(string filename, byte[] content, EpubContentEnum type)
    {
        if (string.IsNullOrWhiteSpace(filename)) throw new ArgumentNullException(nameof(filename));
    ...
    public void AddFile(string filename, string content, EpubContentEnum type)
    {
    ...
    public EpubChapter AddChapter(string title, string html, string fileId = null)
    {
        ...
        var file = new EpubTextFile
        {
            ...
            Href = fileId + ".html",
            TextContent = html,
            ContentType = EpubContentEnum.Xhtml11
        };
        file.MimeType = ContentType.ContentTypeToMimeType[file.ContentType];
    ...
    public void SetCover(byte[] data, ImageFormatEnum imageFormat)
    {
        ...
        string filename;
        EpubContentEnum type;

        switch (imageFormat)
        {
            case ImageFormatEnum.Gif:
                filename = "cover.gif";
                type = EpubContentEnum.ImageGif;
                break;
            case ImageFormatEnum.Jpeg:
                filename = "cover.jpeg";
                type = EpubContentEnum.ImageJpeg;
                break;
            case ImageFormatEnum.Png:
                filename = "cover.png";
                type = EpubContentEnum.ImagePng;
                break;
            case ImageFormatEnum.Svg:
                filename = "cover.svg";
                type = EpubContentEnum.ImageSvg;
                break;
            default:
